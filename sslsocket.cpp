...
static SSL_CTX *get_server_context(std::string &ca_cert,
		std::string &client_cert, std::string &client_key);
static SSL_CTX *get_client_context(std::string &ca_cert,
		std::string &client_cert, std::string &client_key);
...
SSLSocket::connect()
{
	...
	int rc = -1;
	const char *conn_str =
			std::string(addr + ":" + std::to_string(port)).c_str();
...
SSLSocket::accept()
{
	...
	sin_len = sizeof(sin);
	if ((net_fd = ::accept(listen_fd, (struct sockaddr *) &sin, &sin_len)) <
			0) {
	...
	if ((rc = SSL_accept(accepted_ssl)) != 1) {
		fprintf(stderr,
				"Could not perform SSL handshake: %i, %s\n",
				rc,
				strerror(errno));
		if (rc != 0) {
...
static int
get_socket(const std::string &addr, int port_num)
{
	struct sockaddr_in sin;
	int sock, val;
...
SSLSocket::validate_hostname(const char *hostname)
{
	...
	if (server_cert == NULL) {
		// The handshake was successful although the server did not provide a
		// certificate Most likely using an insecure anonymous cipher suite...
		// get out!
